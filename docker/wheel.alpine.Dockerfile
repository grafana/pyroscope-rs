...
#
# Runtime note: the resulting .so requires libgcc_s.so.1 (provided by the
# libgcc Alpine package) because py-spy uses GCC unwind APIs for stack walking.
# Install it with: apk add libgcc
