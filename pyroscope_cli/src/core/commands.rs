...
pub fn exec(command : &Vec<String>, common: &ValidatedArgs) -> Result<()> {
    if command.is_empty() {
...
pub fn connect(pid :i32, common: &ValidatedArgs) -> Result<()> {
    let mut profiler = Profiler::default();
    profiler.init(pid, common)?;
    ...
    println!("Press Ctrl-C to exit."); // TODO why println not log?
