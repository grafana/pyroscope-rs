...
py_rust_version=$(sed -nE 's/^version = "([^"]+)"/\1/p' pyroscope_ffi/python/rust/Cargo.toml)
rb_rust_version=$(sed -nE 's/^version = "([^"]+)"/\1/p' pyroscope_ffi/ruby/ext/rbspy/Cargo.toml)
...
if [[ "$py_cfg_version" != "$py_rust_version" ]]; then
  echo "pyspy version mismatch: pyproject.toml=$py_cfg_version cargo.toml=$py_rust_version"
  exit 1
fi

if [[ "$rb_version" != "$rb_rust_version" ]]; then
  echo "rbspy version mismatch: version.rb=$rb_version cargo.toml=$rb_rust_version"
  exit 1
fi
