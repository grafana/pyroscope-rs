...
Config = namedtuple('Config', ('application_name', 'server_address',
'sample_rate', 'detect_subprocesses','oncpu', 'native', 'gil_only', 'log_level'))

def configure(application_name=None, server_address="http://localhost:4040",
sample_rate=100, detect_subprocesses=False, oncpu=False, native=False,
gil_only=False, log_level="info", tags=None): 
    lib.initialize_agent(application_name.encode("UTF-8"),
            server_address.encode("UTF-8"), sample_rate, detect_subprocesses,
            oncpu, native, gil_only,
